 ...
 * The main program must allocate this structure, one per client, prior to calling
 * the flash server.
 * 
...
/**
 * Function that runs a flash-server. A flash server is a thread that serves one or more
 * clients. There is one flash server per flash-device, and the server can serve clients
 ...
 *
 * This function does, at present, never return. It could be made to return if all clients
 ...
 *                        The header for the client describes the local "filesystem"
 *                        for that client
 * \param n_flash_clients Number of clients. The arrays in the first and
 *                        second parameters should have this many entries
 * \param qspi            Structure holding the quad-flash ports. This contains three
 *                        Ports and a clock-block, the CS_N port, the CLK port, the DATA port
 *                        and a clock block to be used for the flash.
 * \param flash_spec      Array holding specificiations of flash devices, as per the 
 *                        libquadflash documentation
