...
void qpi_reset_port(buffered port:32 p_data, clock clkblk) {
...
static void data_words_out(buffered port:32 p_data, int cycle,
...
static uint32_t data_word_in(buffered port:32 p_data, in port p_cs,
...
static uint32_t code_length = 0;

void qpi_xcore_ai_slave(in port p_cs, in port p_clk,
    ...
    while (running) {
        ...
        case INFERENCE_ENGINE_READ_STATUS:
            data_words_out(p_data, cycle + DUMMY_CLOCKS, status, 0, 1);
            status[0] &= ~STATUS_ERROR;
            break;
        case INFERENCE_ENGINE_READ_ID:
            data_words_out(p_data, cycle + DUMMY_CLOCKS, ai_server_id, 0, 1);
            break;
        case INFERENCE_ENGINE_READ_SPEC:
            data_words_out(p_data, cycle + DUMMY_CLOCKS, ai_server_spec, 0, 1);
            break;
        case INFERENCE_ENGINE_READ_TIMINGS:
            data_words_out(p_data, cycle + DUMMY_CLOCKS, memory, timings_index, timings_length);
            break;
        case INFERENCE_ENGINE_READ_TENSOR:
            data_words_out(p_data, cycle + DUMMY_CLOCKS, memory, output_tensor_index, output_tensor_length);
            break;
