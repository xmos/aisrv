...
void hm0360_monolith_rgb(client interface i2c_master_if i2c, uint8_t * unsafe pt) {
    ...
    printhex((unsigned) rgb4);
    printchar(' ');
    printhex((unsigned) rgb);
    printchar(' ');
    printhex((unsigned) input_image);
    printchar(' ');
    printhexln((unsigned) input_image + BAYER_WIDTH * BAYER_HEIGHT); 
